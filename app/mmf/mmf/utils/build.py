...
import torch
from app.mmf.mmf.common.meter import Meter
from app.mmf.mmf.common.registry import registry
from app.mmf.mmf.datasets.iteration_strategies import (
    ConstantIterationStrategy,
    IterationStrategy,
    SizeProportionalIterationStrategy,
)
from app.mmf.mmf.datasets.processors.processors import Processor
from app.mmf.mmf.utils.configuration import Configuration, get_global_config
from app.mmf.mmf.utils.distributed import is_dist_initialized, is_main, is_xla, synchronize
from app.mmf.mmf.utils.general import get_optimizer_parameters
...
def build_dataset(
    ...
    from app.mmf.mmf.utils.configuration import load_yaml_with_defaults
...
def build_dataloader_and_sampler(
    ...
    """
    from app.mmf.mmf.common.batch_collator import BatchCollator
...
def _add_extra_args_for_dataloader(
    ...
    from app.mmf.mmf.utils.general import get_batch_size
...
def build_classifier_layer(config, *args, **kwargs):
    from app.mmf.mmf.modules.layers import ClassifierLayer
...
def build_text_encoder(config, *args, **kwargs):
    ...
    try:
        from app.mmf.mmf.modules.fb.encoders import TextEncoderFactory
    except ImportError:
        from app.mmf.mmf.modules.encoders import TextEncoderFactory
...
def build_image_encoder(config, direct_features=False, **kwargs):
    """Deprecated, please do not use"""
    from app.mmf.mmf.modules.encoders import ImageEncoderFactory, ImageFeatureEncoderFactory
...
def build_graph_encoder(config):
    from app.mmf.mmf.modules.graphnetwork import Graph_Module

    graph_module = Graph_Module(config)
    return graph_module.module

def build_fusion_module(config):
    from app.mmf.mmf.modules.fusions import Fusion_Module
...
def build_attention_module(config):
    # TODO:
    from app.mmf.mmf.modules.attention import Attention_Module

    attention_module = Attention_Module(config)
    return attention_module.module

def build_classifier(config):
    from app.mmf.mmf.modules.classifiers import Classifier
...
def build_encoder(config: Union[DictConfig, "mmf.modules.encoders.Encoder.Config"]):
    from app.mmf.mmf.modules.encoders import Encoder
